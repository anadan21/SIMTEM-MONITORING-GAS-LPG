...
const DATASETS = {
  ppm:   { label: 'PPM Gas (MQ-6)', color: '#fbbf24', unit: 'ppm', yMax: 5000 },
  ...
  return {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        titleColor: '#8899aa', bodyColor: '#f0f4f8',
        borderColor: 'rgba(255,255,255,0.06)', borderWidth: 1, padding: 10,
...
function initChart() {
  ...
  chartInstance = new Chart(canvas.getContext('2d'), {
    ...
    data: {
      ...
      datasets: [{
        label: cfg.label, data: history[activeTab],
        borderColor: cfg.color, backgroundColor: hexToRGBA(cfg.color, 0.07),
        borderWidth: 2, pointRadius: 2.5, pointHoverRadius: 5,
        pointBackgroundColor: cfg.color, pointBorderColor: 'transparent',
...
function switchTab(tab) {
  ...
  ds.label = cfg.label; ds.data = history[tab];
  ds.borderColor = cfg.color; ds.backgroundColor = hexToRGBA(cfg.color, 0.07);
